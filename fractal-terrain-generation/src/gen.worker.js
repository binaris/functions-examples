 ...
 */
async function genData(ID, size, xPos, zPos, downscale,
  heightFactor, endpoint, numRetries = 1) {
  ...
  for (let i = 0; i < numRetries; i += 1) {
    try {
      ...
      thisWorker.postMessage({
        ...
        zPos,
        heightFactor,
        maxHeight: headers['gen-data-max-height'],
        blockCount: headers['gen-data-blockcount'],
        payloadBytes: headers['gen-data-payload-bytes'],
        funcGenTimeMS: headers['gen-data-time-running-ms'],
...
thisWorker.addEventListener('message', async (e) => {
  ...
  genData(ID, size, xPos, zPos, downscale, heightFactor, endpoint, numRetries);
});
